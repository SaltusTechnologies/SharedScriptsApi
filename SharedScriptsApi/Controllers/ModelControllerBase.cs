...
namespace SharedScriptsApi.Controllers
{
    ...
    //    /// <returns></returns>
    //    protected async Task<IActionResult> ProcessRequest(string description, bool isAnonymous, Func<JToken, Guid, HttpContext, Task<IServiceResponse>> process)
    //    {
    //        IServiceResponse response;
    //        Guid id = Guid.Empty;
    ...
    //                response = await process(requestToken, id, HttpContext);
